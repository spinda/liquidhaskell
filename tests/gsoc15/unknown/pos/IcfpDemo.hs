...
[lq| tfac :: Nat -> n:Nat -> Nat / [n] |]
tfac :: Int -> Int -> Int
tfac x n = if n <= 1 then x
                     else tfac (n*x) (n-1)

[lq| range :: lo:Nat -> hi:Nat -> [Nat] / [hi-lo] |]
range :: Int -> Int -> [Int]
range lo hi
  | lo < hi   = lo : range (lo + 1) hi
  | otherwise = []

[lq| data L [sz] a <p :: L a -> Prop>
      = N | C (x::a) (xs::L <p> a <<p>>)
  |]
data L a = N | C a (L a)

[lq| measure sz  :: L a -> Int
    sz (C x xs) = 1 + (sz xs)
    sz (N)      = 0
  |]
[lq| invariant {v:L a | (sz v) >= 0} |]

[lq| map :: (a -> b) -> xs:L a -> (L b) / [(sz xs)] |]
map :: (a -> b) -> L a -> L b
map f (C x xs) = f x `C` map f xs
map _ N        = N

[lq| merge :: xs:_ -> ys:_ -> _ / [(sz xs) + (sz ys)] |]
merge :: Ord a => L a -> L a -> L a
merge (C x xs) (C y ys)
  | x < y     = x `C` merge xs (y `C` ys)
  | otherwise = y `C` merge (x `C` xs) ys

[lq| measure emp  :: L a -> Prop
    emp (N)      = true
    emp (C x xs) = false
  |]

[lq| type Stream a = {xs: L <{\v -> not (emp v)}> a | not (emp xs)} |]
