...
[lq| measure mlen :: (Map k a) -> Int 
    mlen(Tip)           = 0
    mlen(Bin s k v l r) = 1 + (mlen l) + (mlen r) 
  |]

[lq| invariant {v:Map k a | (mlen v) >=0} |]
...
[lq| type OMap k a = Map <{\root v -> v < root}, {\root v -> v > root}> k a |]
...
[lq| isRoot :: t:Map k a -> {v: Bool | (Prop(v) <=> isBin(t))} |]
isRoot (Bin _ _ _ _ _) = True
isRoot (Tip)           = False

[lq| filterGt :: (Ord k) => x:MaybeS k -> OMap k v -> OMap {v:k | ((isJustS(x)) => (v > fromJustS(x))) } v |]
...
[lq| filterLt :: (Ord k) => x:MaybeS k -> OMap k v -> OMap {v:k | ((isJustS(x)) => (v < fromJustS(x))) } v |]
...
[lq| join :: kcut:k -> a -> OMap {v:k | v < kcut} a -> OMap {v:k| v > kcut} a -> OMap k a |]
