...
[lq| Decrease go 2 |]
[lq| myrev :: xs:[a] -> {v:[a]| (elts v) = (elts xs)} |]
myrev = go [] 
  where 
    go acc []     = acc
...
[lq| myapp :: xs:[a] -> ys:[a] -> {v:[a] | (elts v) = (Set_cup (elts xs) (elts ys))} |]
myapp []     ys = ys
myapp (x:xs) ys = x : myapp xs ys

-- | Finally, to round off this little demo, here's @filter@, which returns a subset.

[lq| myfilter :: (a -> Bool) -> xs:[a] -> {v:[a]| Set_sub (elts v) (elts xs) } |]
myfilter f []     = []
myfilter f (x:xs) 
  | f x           = x : myfilter f xs 
