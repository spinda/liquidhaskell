...
[lq| mergeSort :: (Ord a) => xs:[a] -> {v:OList a| (len v) = (len xs)} |]
mergeSort :: Ord a => [a] -> [a]
mergeSort []  = []
mergeSort [x] = [x]
...
[lq| predicate Pr X Y = (((len Y) > 1) => ((len Y) < (len X))) |]
...
[lq| Decrease merge 4 |]
[lq| merge :: Ord a => xs:(OList a) -> ys:(OList a) -> d:{v:Int| v = (len xs) + (len ys)} -> {v:(OList a) | (len v) = d} |]
