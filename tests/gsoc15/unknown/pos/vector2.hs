...
[lq| predicate InBound I A = ((Ge I 0) && (Lt I (vlen A))) |]

[lq| invariant {v:Int | v >= 0} |]
[lq| unsafeLookup :: vec:Vector a 
                 -> {v: Int | (0 <= v && v < (vlen vec)) } 
                 -> a |]
unsafeLookup vec i = vec ! i

[lq| unsafeLookup' :: vec:Vector a -> {v: Int | (InBound v vec)} -> a |]
unsafeLookup' vec i = vec ! i
...
[lq| absoluteSum   :: Vector Int -> {v: Int | 0 <= v}  |]
...
[lq| type SparseVector a N = [({v: Int | (0 <= v && v < N)}, a)] |]

[lq| sparseDotProduct :: (Num a) => x:(Vector a) -> (SparseVector a {(vlen x)}) -> a |]
