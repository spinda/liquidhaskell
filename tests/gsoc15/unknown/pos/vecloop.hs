...
[lq| data Vec a < dom :: Int -> Prop,
                 rng :: Int -> a -> Prop >
      = V {a :: i:Int<dom> -> a <rng i>}  |]

[lq| empty :: forall <p :: Int -> a -> Prop>. 
               Vec <{v:Int|0=1}, p> a     |]

empty     = V $ \_ -> error "empty vector!"

[lq| set :: forall a <d :: Int -> Prop,
                     r :: Int -> a -> Prop>.
...
[lq| predicate Seg V I J = (I <= V && V < J) |]

[lq| type IdVec N = Vec <{\v -> (Seg v 0 N)}, 
                        {\k v -> v=k}> 
                       Int                  |]

[lq| initUpto :: n:Nat -> (IdVec n) |]
initUpto n   = loop 0 n empty (\i acc -> set i i acc)

[lq| qualif Foo(v:Int): v < 0 |]
