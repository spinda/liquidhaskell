...
[lq| type Matrix a       = {m0:[{v:[a] | (len v) = (cols m0)}] | true } |]

[lq| type MatrixCR a C R = {m0:[{v:[a] | (len v) = C}] | (((len m0) = R) && (R > 0 => (cols m0) = C)) } |]

[lq| measure cols :: [[a]] -> Int
    cols ([])   = 0
    cols (x:xs) = (len x)
  |]
...
[lq| predicate Dim V C R = (((len V) = R) && ((cols V) = C)) |]

[lq| mat_3_2 :: {v: Matrix Int | (Dim v 3 2)} |]
mat_3_2 :: [[Int]]
mat_3_2 = [ [1,2,3]
          , [4,5,6] ]
          
[lq| mat_2_4 :: {v: Matrix Int | (Dim v 2 4)} |]
mat_2_4 :: [[Int]]
mat_2_4 = [ [1,2]
          , [3,4]
          , [5,6]
...
[lq| transpose :: c:Nat -> r:{v:Nat | v > 0}  -> (MatrixCR a c r) -> (MatrixCR a r c) |]
...
[lq| transpose' :: m:{v:(Matrix a) | ((len v) > 0)} -> (MatrixCR a (len m) (cols m)) |]
transpose' :: [[a]] -> [[a]]
transpose' ([]:_)         = []
