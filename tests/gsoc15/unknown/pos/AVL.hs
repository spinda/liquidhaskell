...
-- Basic functions
[lq| data Tree [ht] a = Nil | Tree (x::a) (l::Tree a) (r::Tree a) |]
...
[lq| emp :: {v: AVLTree | ht v == 0} |]
emp = Nil

[lq| singleton :: a -> {v: AVLTree | ht v == 1 }|]
singleton a = Tree a Nil Nil

-- | Insert functions

[lq| Decrease insert 3 |]
[lq| insert :: a -> s: AVLTree -> {t: AVLTree | EqHt t s || HtDiff t s 1 } |]
...
[lq| rebalanceLL :: a -> l:{AVLTree | LeftHeavy l } -> r:{AVLTree | HtDiff l r 2} -> {t:AVLTree | EqHt t l } |]
rebalanceLL v (Tree lv ll lr) r                 = Tree lv ll (Tree v lr r)

[lq| rebalanceLR :: a -> l:{AVLTree | RightHeavy l } -> r:{AVLTree | HtDiff l r 2 } -> {t: AVLTree | EqHt t l } |]
rebalanceLR v (Tree lv ll (Tree lrv lrl lrr)) r = Tree lrv (Tree lv ll lrl) (Tree v lrr r)

[lq| rebalanceRR :: a -> l: AVLTree -> r: {AVLTree | RightHeavy r && HtDiff r l 2 } -> {t: AVLTree | EqHt t r } |]
rebalanceRR v l (Tree rv rl rr)                 = Tree rv (Tree v l rl) rr

[lq| rebalanceRL :: a -> l: AVLTree -> r:{AVLTree | LeftHeavy r && HtDiff r l 2} -> {t: AVLTree | EqHt t r } |]
...
[lq| type AVLTree = {v: Tree a | balanced v} |]
