...
[lq| add :: (Ord a) => a -> ORBT a -> ORBT a |]
add x s = makeBlack (ins x s)

[lq| ins :: (Ord a) => a -> ORBT a -> ORBT a |]
...
[lq| remove :: (Ord a) => a -> ORBT a -> ORBT a |]
remove x t = makeBlack (del x t)

[lq| del              :: (Ord a) => a -> ORBT a -> ORBT a |]
del x Leaf           = Leaf
...
[lq| append  :: y:a -> ORBT {v:a | v < y} -> ORBT {v:a | y < v} -> ORBT a |]
...
[lq| deleteMin' :: k:a -> ORBT {v:a | v < k} -> ORBT {v:a | k < v} -> (a, ORBT a) |]
