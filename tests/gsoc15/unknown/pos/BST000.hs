...
[lq| measure blen :: (Bst k v) -> Int
    blen(Empty)        = 0
    blen(Bind k v l r) = 1 + (blen l) + (blen r)
  |]

[lq| invariant {v:Bst k v | (blen v) >= 0} |]

data Bst k v = Empty | Bind k v (Bst k v) (Bst k v)

[lq| type OBST k a = Bst <{\root v -> v < root }, {\root v ->  v > root}> k a |]

[lq| chkMin :: (Ord k) => x:k -> OBST {v:k | x < v} a -> Bool |]
...
[lq| propMin :: (Ord k) => OBST k a -> Bool |]
