...
------------------------------------------------------------------
[lq| lookup :: x:Bndr -> {v:Env | Set_mem x (vars v)} -> Val |]
lookup :: Bndr -> Env -> Val
---------------------  -------------------------------------------
lookup x ((y,v):env)   
  | x == y             = v
...
[lq| type CExpr G = {v:Expr | Set_sub (free v) (vars G)} |]

[lq| measure vars :: Env -> (Set Bndr)
    vars ([])    = {v | Set_emp v }
    vars (b:env) = {v | v = Set_cup (Set_sng (fst b)) (vars env)}
  |]

[lq| measure free       :: Expr -> (Set Bndr) 
