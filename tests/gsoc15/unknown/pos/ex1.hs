...
[lq| data Vec [llen] a = Nil | Cons (x::a) (xs::(Vec a)) |]

-- | We can encode the notion of length as an inductive measure @llen@

[lq| measure llen     :: forall a. Vec a -> Int
    llen (Nil)       = 0
    llen (Cons x xs) = 1 + llen(xs)
  |]

[lq| invariant {v:Vec a | (llen v) >= 0} |]

-- | As a warmup, lets check that a /real/ length function indeed computes
-- the length of the list.

[lq| sizeOf :: xs:Vec a -> {v: Int | v = llen xs} |]
sizeOf             :: Vec a -> Int
sizeOf Nil         = 0
...
[lq| efoldr :: forall a b <p :: x0:Vec a -> x1:b -> Prop>.
