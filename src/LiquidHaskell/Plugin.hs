...
import Language.Haskell.Liquid.GhcMisc as GM
...
pluginMain cfg = unlessInteractive $ whenNoErrs $ do
  mod     <- tcPluginModule
  summary <- tcPluginGhc $ getModSummary $ moduleName mod
...
-- | Get the current module being checked in the TcPluginM monad
tcPluginModule :: TcPluginM Module
tcPluginModule = (tcg_mod . fst) <$> getEnvs

-- | Get the current DynFlags in the TcPluginM monad
