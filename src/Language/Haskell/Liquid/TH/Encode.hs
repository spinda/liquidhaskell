...
module Language.Haskell.Liquid.TH.Encode (
    -- * AST Newtypes
    Reft
  , Pred
  , Expr
...
import Language.Haskell.Liquid.TH.Types hiding (Expr, Pred)
...
_tupT :: (Type, Type) -> Type
_tupT (x, y) = PromotedTupleT 2 `AppT` x `AppT` y
...
maybeE :: Maybe Exp -> Exp
maybeE Nothing  = ConE 'Nothing
maybeE (Just e) = ConE 'Just `AppE` e
...
annIsInline :: Located Name -> Maybe (Located String) -> Dec
annIsInline = annIsLogic 'IsInline

annIsBound :: Located Name -> Maybe (Located String) -> Dec
annIsBound = annIsLogic 'IsBound

annIsMeasure :: Located Name -> Maybe (Located String) -> Dec
annIsMeasure = annIsLogic 'IsMeasure


annIsLogic :: Name -> Located Name -> Maybe (Located String) -> Dec
annIsLogic ann var alias =
  PragmaD $ AnnP (ValueAnnotation $ val var) $
    ConE ann `AppE` spanE (loc_span var) `AppE` maybeE (locE . fmap strE <$> alias)
