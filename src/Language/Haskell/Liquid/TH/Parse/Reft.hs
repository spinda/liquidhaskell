module Language.Haskell.Liquid.TH.Parse.Reft (
    reftP
  , predP
  , exprP
  ) where
...
import Language.Haskell.Liquid.TH.Types (Brel(..), Bop(..))
...
etermP = do
  ...
  return $ case es of
    [e]     -> val e
    (e:es') -> eApp e (val <$> es')
    []      -> error "etermP: empty list from many1"
...
eVarP = do
  name <- qual $ choice
    [ varidP <?> "variable or binder"
    , conidP <?> "data constructor or expression parameter"
    ]
  ep <- isExprParam name
  return $ (if ep then eParam else eVar) name
